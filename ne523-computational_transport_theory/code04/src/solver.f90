...
print*, "----------------------------"
print*, " Beginning Inner Iterations "
print*, "----------------------------"
call inner()
print*, "---------------------------"
print*, " Inner Iterations Complete "
print*, "---------------------------"
...
do k = 1, angles_per_octant
    ...
    do y_id = 1, n_y_cells
        ...
        !do x_id = n_x_cells, 1, -1
        do x_id = 1, n_x_cells
            dx = x_cell_sizes(x_id)
            source = distrib_source(x_id, y_id)
...
SUBROUTINE inner()
    ...
    ! Conducts the inner iterations.
    !
    ! Conducts the inner iterations by taking the an initial guess for the
    ! scalar flux (the first guess is an array of 0), performing a mesh sweep,
    ! calculating a new source distribution by summing the fixed source and the
    ! scattering source, calculating the L-inf norm between the previous flux
    ! iterate and the current flux iterate, and evaluating convergence criterion
    !
    ! These parameters are not directly passed around by inner, but all relevant
    ! variables will be described here nonetheless
    !
    ! Parameters
    ! ----------
    !  old_flux_iterate : scalar flux before the current mesh sweep
    !  scalar_flux_array : scalar flux after the current mesh sweep
    !  material_id : material of the current cell
    !  scattering_xs : scattering cross section of the current cell
    !  fixed_source : fixed source of the current cell
    !  scatt_source : scattering course of the current cell
    !  rel_diff : abs of difference between previous and updated flux of current cell
    !  linf_diff : L-infinity norm max `rel_diff` for the current sweep

    real, dimension(n_x_cells, n_y_cells) :: old_flux_iterate
    integer :: inner_iter_index, x_id, y_id, material_id
    real :: scattering_xs, fixed_source, scatt_source, linf_err, rel_diff

    do inner_iter_index = 1, max_inner_iters
        print*, ""
        print "(A, I0)", " Beginning inner iteration ", inner_iter_index
        old_flux_iterate = scalar_flux_array

        print *, "  Calculating distributed source"
        do y_id = 1, n_y_cells
            do x_id = 1, n_x_cells
                material_id = material_array(x_id, y_id)
                scattering_xs = cross_sections(2, material_id)

                fixed_source = source_array(x_id, y_id)
                scatt_source = old_flux_iterate(x_id, y_id) * scattering_xs

                distrib_source(x_id, y_id) = fixed_source + scatt_source
            end do
        end do


        print *, "  Performing mesh sweep"
        call sweep()


        print *, "  Evaluating L-inf norm"
        linf_err = 0
        do y_id = 1, n_y_cells
            do x_id = 1, n_x_cells
                rel_diff = abs(scalar_flux_array(x_id, y_id) / old_flux_iterate(x_id, y_id) - 1)
                if (rel_diff > linf_err) then
                    linf_err = rel_diff
                end if
            end do
        end do

        if (linf_err <= convergence_criterion) then
            print*, new_line("")
            print*, "Successfully Reached Convergence Criterion."
            print "(A, I0, A)", "   Converged in ", inner_iter_index, " inner iterations"
            print "(A, Es12.4, A, Es12.4)", "   L-inf norm of", linf_err, " <= convergence criterion of", convergence_criterion
            exit
        end if

        if (inner_iter_index == max_inner_iters) then
            print*, new_line("")
            print*, "Unsuccessfully Reached Convergence Criterion."
            print "(A, I0, A)", "   Failed to converge after ", max_inner_iters, " inner iterations"
            print "(A, Es12.4, A, Es12.4)", "   L-inf norm of", linf_err, " > convergence criterion of", convergence_criterion
        end if
    end do
