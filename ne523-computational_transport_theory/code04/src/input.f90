...
SUBROUTINE input_data()
    ...
    allocate(distrib_source(n_x_cells, n_y_cells))

    allocate(scalar_flux_array(n_x_cells, n_y_cells))
    scalar_flux_array = 0

    read(input_unit, *) material_array
    read(input_unit, *) source_array

    read(input_unit, *) convergence_criterion, max_inner_iters
...
SUBROUTINE input_check()
    ...
    call validate_convergence_criterion()
    call validate_max_inner_iters()
...
SUBROUTINE validate_boundary_conditions()
    ...
    do i = 1, 4
        if (boundary_conditions(i) .ne. 0) then
            call throw_error("All BCs need to be vacuum (0)")
        end if
    end do
    print*, "    Passed all checks"
...
SUBROUTINE validate_convergence_criterion()
    print*, "Validating convergence criterion"
    call check_positive(convergence_criterion)
    print*, "    Passed all checks"
END SUBROUTINE validate_convergence_criterion


SUBROUTINE validate_max_inner_iters()
    print*, "Validating max inner iterations"
    call check_positive(max_inner_iters)
    print*, "    Passed all checks"
END SUBROUTINE validate_max_inner_iters
